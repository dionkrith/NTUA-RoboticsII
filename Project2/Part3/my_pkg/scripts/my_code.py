...
if __name__ == '__main__':
	...
	#Create the occupancy grid map that is included in costmap list

	costmap = [254 for i in range(80)]
	for j in range(19): #instead of 29
    		costmap = costmap + [254] + [0 for i in range(78)] + [254]
    
	for j in range(10):
    		costmap = costmap + [254] + [0 for i in range(24)] 
        
    		for i in range(20):
        		costmap.append(254)
    
		costmap = costmap + [0 for i in range(34)] + [254]
    
	for j in range(20): 
    		costmap = costmap + [254] + [0 for i in range(14)]
    		...
    		for i in range(20):
			temp_list.append(len(costmap)+1)
        		costmap.append(254)
        
    		for i in range(10): 
			costmap.append(254) 
    
    		costmap = costmap +[0 for i in range(24)] + [254]
	...
	costmap = costmap + [254 for i in range(80)]
	...
	for node in temp_list:
    		a = indexToWorld(node,width, resolution, origin)
    		y_temp.append(a[0])
    		x_temp.append(a[1])
	...
	if not path:
		...
		fig = plt.figure()
    		...
    		ax1.scatter(x_temp, y_temp, c ='orange', label='obstacle')
    		ax1.scatter(x_result,y_result, c ='green', label='path')
    		plt.legend(loc='upper left');
    		plt.show()

		pub = rospy.Publisher("/mymobibot/cmd_vel",Twist,queue_size=1)
		sub =  rospy.Subscriber("/mymobibot/odom",Odometry,callback)
		cmd = Twist()
		rate = rospy.Rate(5)
		x_list = []
		y_list = []
		...
		for i in range(len(result_list)):
			...
			error = [result_list[i][1] -x , result_list[i][0]-y]
			while(abs(error[0])>0.1 or abs(error[1])>0.1):
			
				error = [result_list[i][1] -x , result_list[i][0]-y]

				error_list_x.append(error[0])
				error_list_y.append(error[1])
				result_temp_x.append(result_list[i][1])
				result_temp_y.append(result_list[i][0])
				x_list.append(x)
				y_list.append(y)
				if (x_prev == result_list[i][1] or y_prev == result_list[i][0]):
					if (abs( abs(theta)-np.pi/2 ) <= 0.05 or abs(theta) <= 0.05):
						
						straight_line = True
						v_in = k1*np.sqrt(error[0]**2 + error[1]**2)
						w_in = 0
					else:
						w_in =  k2*(np.arctan2(-error[1],-error[0]) - theta)
					...
					w_in =  k2*(np.arctan2(-error[1],-error[0]) - theta)
					if (straight_line == True and count<2):
						count = count + 1
						non_straight = True
						w_in = -w_in
					v_in = k1*np.sqrt(error[0]**2 + error[1]**2)	
			...
			x_prev = result_list[i][1]
			y_prev = result_list[i][0]
			if (t == 140): break
			if (non_straight ==True and straight_line == True):
				non_straight = False
				straight_line = False 
		...
		fig = plt.figure()
    		ax1 = fig.add_subplot(111)
    		ax1.scatter(x_temp, y_temp, c ='orange', label='obstacle')
		ax1.scatter(x_list,y_list, c ='blue', label='robot movement')
    		plt.legend(loc='upper left');
    		plt.show()
